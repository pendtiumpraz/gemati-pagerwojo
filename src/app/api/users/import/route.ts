...
// POST /api/users/import — import massal pengguna (role dari row) (body: { rows: [...] })
export async function POST(req: NextRequest) {
  return handle(async () => {
    const session = await requireAuth(["admin"]);
    const body = await req.json().catch(() => null);
    if (!body || !Array.isArray(body.rows)) return bad("Format tidak valid");
    const result = await importUsers(body.rows);
    await logAudit({
      user: session,
      aksi: "Import Data",
      modul: "Pengguna",
      detail: `Import ${result.inserted} pengguna (${result.failed} gagal)`,
    });
